...
const ProductList = ({ product, setProduct }) => {
    ...
    const filtterproduct = (product) => {
        const update = dummyData.filter((x) => {
    ...
    // Function to fetch product list (replace with actual fetch logic)
    const dummyData = [
        {
            id: "1",
            name: "Mug Printing",
            ...
            <section id="prod">
                <div className="row mx-auto container">
                    {product.map((curElm) => (
                        ...
                        <ProductDisplay product={curElm} setProduct={setProduct} key={curElm.id} />
                    ))}
                </div>
            </section>
        </section>
